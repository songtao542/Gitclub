...
import com.aperise.gitclub.settings.SettingsState.Setting;
...
public class SettingsProvider extends ContentProvider {
    ...
    private static final Bundle NULL_SETTING = forPair(Settings.VALUE, null);
    ...
    public Bundle call(String method, String name, Bundle args) {
        switch (method) {
            case Settings.CALL_METHOD_GET: {
                Setting setting = getSetting(name);
                return packageValueForCallResult(setting);
            }
            case Settings.CALL_METHOD_PUT: {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        String name = values.getAsString(Settings.NAME);
        if (!isKeyValid(name)) {
            return null;
        }
        String value = values.getAsString(Settings.VALUE);
        if (insertSetting(name, value)) {
            return Uri.withAppendedPath(Settings.CONTENT_URI, name);
        }
        return null;
    }
    ...
    private void dumpSettings(Cursor cursor, PrintWriter pw) {
        ...
        final int idColumnIdx = cursor.getColumnIndex(Settings._ID);
        final int nameColumnIdx = cursor.getColumnIndex(Settings.NAME);
        final int valueColumnIdx = cursor.getColumnIndex(Settings.VALUE);
...
//        enforceWritePermission(Manifest.permission.WRITE_SECURE_SETTINGS);
    ...
    private static Bundle packageValueForCallResult(Setting setting) {
        ...
        return forPair(Settings.VALUE, setting.getValue());
    }

    private static String getSettingValue(Bundle args) {
        return (args != null) ? args.getString(Settings.VALUE) : null;
    }
    ...
    final class SettingsRegistry {
        ...
        private Uri getNotificationUriFor(String name) {
            return (name != null) ? Uri.withAppendedPath(Settings.CONTENT_URI, name) : Settings.CONTENT_URI;
        }
