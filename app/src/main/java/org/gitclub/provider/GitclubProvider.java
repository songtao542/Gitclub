...
public class GitclubProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case ACCESS_TOKEN:
                return db.delete(AccessToken.TABLE_NAME, selection, selectionArgs);
            case ACCESS_TOKEN_ID:
                longId = Long.parseLong(uri.getPathSegments().get(1));
                SLog.d(TAG, "delete ACCESS_TOKEN_ID parameter id=" + longId);
                return db.delete(AccessToken.TABLE_NAME, AccessTokenColumns._ID + "=" + longId + " AND (" + selection + ")", selectionArgs);
            case USER:
                return db.delete(User.TABLE_NAME, selection, selectionArgs);
            case USER_ID:
                longId = Long.parseLong(uri.getPathSegments().get(1));
                SLog.d(TAG, "delete USER_ID parameter id=" + longId);
                return db.delete(User.TABLE_NAME, UserColumns._ID + "=" + longId + " AND (" + selection + ")", selectionArgs);
            case USER_NAME:
                String name = uri.getLastPathSegment();
                SLog.d(TAG, "delete USER_NAME parameter name=" + name);
                return db.delete(User.TABLE_NAME, UserColumns.LOGIN + "=" + name + " AND (" + selection + ")", selectionArgs);
            case USER_EMAIL:
                String email = uri.getLastPathSegment();
                SLog.d(TAG, "delete USER_EMAIL parameter email=" + email);
                return db.delete(User.TABLE_NAME, UserColumns.EMAIL + "=" + email + " AND (" + selection + ")", selectionArgs);
            default:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case ACCESS_TOKEN:
                longId = db.insert(AccessToken.TABLE_NAME, null, values);
                SLog.d(TAG, "insert ACCESS_TOKEN row id=" + longId);
                resultUri = ContentUris.withAppendedId(AccessToken.CONTENT_URI, longId);
                break;
            case ACCESS_TOKEN_ID:
                longId = Long.parseLong(uri.getPathSegments().get(1));
                SLog.d(TAG, "insert ACCESS_TOKEN_ID parameter id=" + longId);
                values.put(AccessTokenColumns._ID, longId);
                insert(AccessToken.CONTENT_URI, values);
                resultUri = ContentUris.withAppendedId(AccessToken.CONTENT_URI, longId);
                break;
            case USER:
                longId = db.insert(User.TABLE_NAME, null, values);
                SLog.d(TAG, "insert USER row id=" + longId);
                resultUri = ContentUris.withAppendedId(User.CONTENT_URI, longId);
                break;
            case USER_ID:
                longId = Long.parseLong(uri.getPathSegments().get(1));
                SLog.d(TAG, "insert USER_ID parameter id=" + longId);
                values.put(UserColumns._ID, longId);
                resultUri = insert(User.CONTENT_URI, values);
                break;
            case USER_NAME:
                String name = uri.getLastPathSegment();
                SLog.d(TAG, "insert USER_NAME parameter name=" + name);
                values.put(UserColumns.LOGIN, name);
                resultUri = insert(User.CONTENT_URI, values);
                break;
            case USER_EMAIL:
                String email = uri.getLastPathSegment();
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (match) {
            case ACCESS_TOKEN:
                return db.query(AccessToken.TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder, limit);
            case ACCESS_TOKEN_ID:
                longId = Long.parseLong(uri.getPathSegments().get(1));
                SLog.d(TAG, "query ACCESS_TOKEN_ID parameter id=" + longId);
                return db.query(AccessToken.TABLE_NAME, projection, AccessTokenColumns._ID + "='" + longId + "'" + (selection != null ? (" AND (" + selection + ")") : ""),
                        selectionArgs, null, null, sortOrder, limit);
            case USER:
                return db.query(User.TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder, limit);
            case USER_ID:
                longId = Long.parseLong(uri.getPathSegments().get(1));
                SLog.d(TAG, "query USER_ID parameter id=" + longId);
                return db.query(User.TABLE_NAME, projection, UserColumns._ID + "='" + longId + "'" + (selection != null ? (" AND (" + selection + ")") : ""),
                        selectionArgs, null, null, sortOrder, limit);
            case USER_NAME:
                String name = uri.getLastPathSegment();
                SLog.d(TAG, "query USER_NAME parameter name=" + name);
                return db.query(User.TABLE_NAME, projection, UserColumns.LOGIN + "='" + name + "'" + (selection != null ? (" AND (" + selection + ")") : ""),
                        selectionArgs, null, null, sortOrder, limit);
            case USER_EMAIL:
                String email = uri.getLastPathSegment();
                SLog.d(TAG, "query USER_EMAIL parameter email=" + email);
                return db.query(User.TABLE_NAME, projection, UserColumns.EMAIL + "='" + email + "'" + (selection != null ? (" AND (" + selection + ")") : ""),
                        selectionArgs, null, null, sortOrder, limit);
            default:
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        switch (match) {
            case ACCESS_TOKEN:
                return db.update(AccessToken.TABLE_NAME, values, selection, selectionArgs);
            case ACCESS_TOKEN_ID:
                longId = Long.parseLong(uri.getPathSegments().get(1));
